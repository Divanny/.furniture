<template>
    <Button
        :icon="$store.state.favoritesProducts.some(x => x.idProducto == idProducto) ? 'fa-solid fa-heart text-red-400' : 'fa-regular fa-heart'"
        size="small" style="padding: 0px 12px; width: 2rem; height: 2rem;" class="bg-white border-none text-black-alpha-90"
        rounded severity="secondary" @click="manageFavorite()" />
</template>
<script>
